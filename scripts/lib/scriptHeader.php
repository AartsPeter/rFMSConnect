<?php
    ...
    if ($SCNumber!='*') {
		...
		$str5=" AND gvc.accountnumber='".$SCNumber."'";
    }
    ...
    if(isset($_GET['endDate'])) 	{ $ED		= test_input($_GET['endDate']);}	else { $ED = date("Y-m-d 23:59:59");}
 #   if(isset($_GET['startDate'])) 	{ $SD		= test_input($_GET['startDate']);} 	else { $SD = date('Y-m-d',strtotime('-'.$days.' day'));}
    ...
    if (isset($_GET['StartDate'])) 	{ $SD		= test_input($_GET['StartDate']);}
		...
		$SD1	= date('Y-m-d',strtotime('1 Monday ago'));
	}
	if (isset($_GET['today'])) 		{ $SD		= date("Y-m-d 00:00:00");$ED = date("Y-m-d 23:59:59");}
